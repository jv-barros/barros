...
sp_folder_superquadra_entradas = site.Folder('Documentos Compartilhados/Power BI/Superquadra/Vei Chico (sem data)')
...
# #this function needs be changed to catch correct day 
...
#                     # Verify that the day belongs to the current month
#                     aria_label = dia_elemento.get_attribute("aria-label")
#                     if current_month in aria_label and str(current_year) in aria_label:
...
def execute_script():
    ...
    login_zigpay_spq(navegador, "https://multiloja.zigpay.com.br/login", "veichico.mane", "B3550@=")
    baixar_relatorio_spq_mane(navegador)
    time.sleep(30)
    ...
    new_file_name = f"VeiChico sem data {current_date}.xlsx"
